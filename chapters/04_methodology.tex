...
The \acs{Wasm} variants use the standardised \texttt{wasi:http/incoming-handler} Component Model interface (\acs{WASI}~P2) via Fermyon Spin and Wasmtime/Cranelift. This design choice reflects the current production-ready trajectory of \acs{Wasm} on \acs{K8s}: \acs{WASI}~P2's component model eliminates the proprietary socket workarounds required by \acs{WASI}~P1. For historical context on the \acs{WASI}~P1 approach (WasmEdge, proprietary socket \acs{ABI}), including its benchmark results and implementation constraints, see Appendix~\ref{app:wasmedge_p1}.
...
For historical context on the \acs{WASI}~P1 ecosystem constraints (unmaintained \texttt{hyper\_wasi}/\texttt{tokio\_wasi} forks, \texttt{wasmedge\_wasi\_socket} proprietary crate, single-threaded \texttt{accept()} loop), see Appendix~\ref{app:wasmedge_p1}.
