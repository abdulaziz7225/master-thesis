...
The \acs{CPU} chart for the prime sieve in Figure~\ref{fig:cpu_ps_lim} highlights a complementary trade-off: docker-rust averages ~1212\,mcores during the load window---the highest of any variant---because Tokio spawns multiple OS threads to service the request stream, whereas wasm-rust achieves comparable throughput at ~414\,mcores under the same hold-phase load. The Wasm variants therefore pay a memory-footprint premium but a \acs{CPU}-efficiency dividend, while the Docker variants invert that trade-off.
