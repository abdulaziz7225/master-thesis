...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/iaas_paas_faas_saas.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/k8s_architecture.pdf}
...
Wasm functions as an agnostic hardware abstraction layer. Developers write code in systems languages like C, C++, or Rust, and compile it down to \texttt{.wasm} binaries. The architecture relies on several core principles:
\begin{itemize}
    ...
    \item \textbf{Hardware Agnosticism:} Because it utilizes a virtual stack machine, the compiled bytecode makes no assumptions about the underlying physical CPU architecture (x86\_64, ARM64), enabling true portability.
    \item \textbf{Linear Memory:} Wasm operates in a contiguous, resizable array of bytes. Code executing within the Wasm module cannot execute arbitrary pointers or access memory outside this strictly enforced boundary.
    \item \textbf{Capability-Based Security:} Wasm operates on a strict "deny-by-default" model. The host environment is entirely hidden. The module cannot access networks, the filesystem, or environment variables unless explicitly imported and granted by the host runtime.
\end{itemize}
...
For languages requiring automatic memory management (like Go, Python, or PHP), Wasm historically required the entire language runtime and garbage collector to be compiled and bundled inside the \texttt{.wasm} binary. Ongoing developments, such as the WasmGC proposal, aim to delegate garbage collection to the host environment to mitigate this overhead.

\subsection{The Server-Side Evolution: WASI}
While Wasm successfully delivered near-native performance for computationally heavy browser applications, its portability, security model, and minimal footprint rapidly garnered interest for backend deployment. As Solomon Hykes, co-founder of Docker, noted, the existence of Wasm and WASI in 2008 would have negated the need to create Docker \cite{hykes2019tweet}.

Because Wasm is entirely isolated by default, a bare Wasm module running on a server cannot open a network socket or read a file. In 2019, the WebAssembly System Interface (WASI) was introduced to solve this. WASI defines a standardized API specification that provides Wasm modules with secure, granular, capability-based access to the host operating system \cite{clark2019standardizing}. By acting as a POSIX-like layer, WASI transforms Wasm from a sandboxed browser calculator into a fully functional server-side technology.

\subsection{Wasm vs. Traditional Containers}
Applying Wasm and WASI to the server yields a deployment artifact that sits between a traditional VM and an OCI container, offering unique operational characteristics:
...
\begin{enumerate}
    ...
    \item \textbf{Security and Density:} Wasm utilizes Software-Based Fault Isolation (SFI). If a Wasm module is compromised, the attacker only controls the isolated linear memory sandbox. The minimal memory footprint of Wasm allows Kubernetes nodes to achieve exponentially higher workload density.
    \item \textbf{Performance Trade-offs:} Despite its advantages in density and initialization, Wasm is structurally immature compared to Docker regarding concurrency. Wasm currently lacks standardized multithreading capabilities. Sondell (2024) demonstrated that in high-concurrency HTTP server scenarios, a Docker container can process requests significantly faster because it can scale across multiple CPU cores, whereas a single Wasm instance remains bottlenecked by single-threaded execution \cite{sondell2024performance}. Furthermore, Spies and Mock found that single-threaded Wasm apps perform approximately 14\% slower than native x86\_64 binaries \cite{spies2021evaluation}.
\end{enumerate}

By utilizing WASI shims, Kubernetes can now orchestrate these high-density Wasm workloads directly alongside traditional Docker containers, setting the stage for the comparative analysis executed in this research.