...
Make sure you use an up-to-date LaTeX distribution such as \emph{TeX~Live}.\footnote{\url{https://www.tug.org/texlive/}}
...
In the source file, however, line breaks have proven to simplify editing considerably: putting exactly one sentence per line makes it very easy to move content around, it helps quickly identifying overly long sentences, and allows line-based diff tools to work as expected.
...
Use \emph{math} mode for typesetting formulas.
This will interpret all numbers as quantities and all letters as single-letter variable names, selecting an upright font for numbers and an italic font (and adding some space) for letters.
This is ideal for simply typing out equations that only consist of numbers and single-letter variables, e.g., $x = 2ab$.
However, this interpretation also means that care is required when typesetting anything else -- as it is generally assumed that whitespace between variables means implied multiplication and that italic letters represent variables, while upright letters represent units or descriptive terms.\footnote{\url{https://www.nist.gov/pml/special-publication-811/nist-guide-si-chapter-10-more-printing-and-using-symbols-and-numbers}}
Consequently:

\begin{description}
\item[multi-letter variables] require the use of the \verb|\mathit| command to typeset them with the correct font and spacing, so that $x = fiji$ is correctly formatted as $x = \mathit{fiji}$.

\item[descriptive terms] should be typeset using the \verb|\mathrm| command, so that $x = y_{direct}$ and its abbreviation $x = y_d$ are correctly formatted as $x = y_{\mathrm{direct}}$ and $x = y_\mathrm{d}$, respectively.
Note the difference between typesetting \enquote{$y_d$} if $d$ represents a quantity (e.g., the index 0) and \enquote{$y_\mathrm{d}$} if $\mathrm{d}$ is a descriptive term (e.g., short for \enquote{direct}).

\item[descriptive text] requires the use of the \verb|\text| command to typeset it with the correct (non-math) font and spacing, so that \enquote{$x = 2ab\ (for\ first\ a,b \in \mathbb{R})$} or \enquote{$x = 2ab \mathrm{\  (for\ first\ } a,b \in \mathbb{R} \mathrm{)}$} are correctly formatted as \enquote{$x = 2ab \text{ (for first } a,b \in \mathbb{R} \text{)}$}.

\item[textual operators] require the use of, e.g., the \verb|\mathop| (and \verb|\mathrm|) commands to typeset them with the correct font and spacing, so that $x = z sin(y)$ is correctly formatted as $x = z\mathop{\mathrm{sin}}(y)$.
A more convenient alternative is the use of the \verb|\operatorname| command, resulting in $x = z\operatorname{sin}(y)$.
For frequently used operators, consider defining a new operator with the \verb|\DeclareMathOperator| command, resulting in $x = z\sin(y)$.

\item[units] should be typeset using the \verb|\SI| command, so that $x = 5m/s$ is correctly formatted as $x = \SI{5}{\meter\per\second}$ (see the corresponding section).
...
To give an example, $\zeta(t) = \min\bigl( \zeta_{**} \left( t \right) \bigr)$ is much better written as
\begin{equation}\label{eq:zeta}
\zeta(t) = \min\bigl( \zeta_{**} \left( t \right) \bigr)
,
\end{equation}
...
s_\mathrm{max} &=
    \max\left(
        \left(t_{i+1} - t_i\right)
            : \zeta\left(t_i\right) < 1, i \in \left[ 0, |T|-1 \right]
    \right)
,\\
\psi_{\mathrm{s}}\left(t\right) &=
    \begin{cases}
        \dfrac{\Delta t_{\mathrm{s}}}{s_\mathrm{max}}
            & \text{if $s_\mathrm{max} > 0$}, \\
        1
            & \text{if $s_\mathrm{max} = 0$},
    \end{cases}
\\
\zeta_{\mathrm{s}}\left(t\right) &=
    \frac{
        \psi_{\mathrm{s}} - l_{\mathrm{s}}
    }{
        c_{\mathrm{s}} - l_{\mathrm{s}}
    }
.
\end{align}

The \emph{User's Guide for the amsmath Package}\footnote{\url{https://mirrors.ctan.org/macros/latex/required/amsmath/amsldoc.pdf}}, provided by the American Mathematical Society, has a comprehensive overview of best practices for typesetting mathematical content.
...
For example: \enquote{the measurements show that the car was accelerating at \SI{5}{\meter\per\second\squared} until it reached its final speed of \SI{100}{\kilo\meter\per\hour}.
During this it sent \SI{512}{\bit} long packets for a total of \SI{1}{\mega\byte\per\second}, more than could be decoded.}
...
In brief, use references whenever you cite from related work (either directly or indirectly), or when you build on related work (this includes their way of illustrating a particular concept, in text form as well as in the overall design of a figure).
Also use references to point a reader to related work.
Clearly distinguish between these uses.

Use quotation marks (\enquote{...}) for direct citations (and direct citations only).
Using direct citations where an indirect one would be more appropriate is bad scientific writing.
...
Thus, any can be used.}}~\cite{knuth1984complexity,berry2011apparent}
\item According to~\cite{knuth1984complexity} and~\cite{berry2011apparent}, \textbf{foo and bar are equally-valued; from this the authors conclude that any can be used}.
\end{itemize}
versus
\begin{itemize}
\item \enquote{\textbf{Foo and bar are of equal value}}~\cite{knuth1984complexity,berry2011apparent}.
Thus, any can be used.
\item According to~\cite{knuth1984complexity} and~\cite{berry2011apparent}, \textbf{foo and bar are equally-valued}.
From this it follows that any can be used.
\end{itemize}
versus
\begin{itemize}
\item Foo and \textbf{bar}~\cite{knuth1984complexity,berry2011apparent} are of equal value.
Thus, any can be used.
\item Foo and \textbf{bar} (detailed in~\cite{knuth1984complexity} and~\cite{berry2011apparent}) are of equal value.
Thus, any can be used.
\end{itemize}
versus
\begin{itemize}
\item \textbf{Foo}~\cite{knuth1984complexity} and \textbf{bar}~\cite{berry2011apparent} are of equal value.
Thus, any can be used.
\item \textbf{Foo} (detailed in~\cite{knuth1984complexity}) and \textbf{bar} (detailed in~\cite{berry2011apparent}) are of equal value.
Thus, any can be used.
\end{itemize}
...
\emph{According to \textcite{knuth1984complexity}, Foo and Bar can be characterized as follows.
They are big.
They are bright.
The authors further argue that one can be substituted for the other.
In the following I will go on to prove that this is not true.}
...
Do not cite URLs
Content found there is not peer reviewed and it is likely to change during the lifetime of your work.
...
Instead of \emph{The benefits of cement filled tires has been shown by \cite{hardes2019communication}}, consider writing \emph{\textcite{hardes2019communication} have shown the benefits of cement filled tires}.
...
Check the page numbers (many digital libraries put something like \enquote{1--5} here despite the paper starting at a later page -- or despite it not having any page numbers to begin with).
...
Check articles and capitalization (see, e.g., references \cite{nikoli2008would,moulin2021the} for examples.
